...
async function syncUser(req, res) {
    try {
        ...
        // Check if user already exists by email (users table has no organization_id)
        const [existing] = await db.query(
            'SELECT u.id, u.email, e.organization_id FROM users u LEFT JOIN employees e ON e.user_id = u.id WHERE u.email = ? LIMIT 1',
            [email]
        );
        ...
        // Create new user (users table doesn't have organization_id — that's on employees)
...
async function unsyncUser(req, res) {
    try {
        ...
        // Find user by empcloud_user_id (join employees for org info)
...
async function getAvailableFromEmpCloud(req, res) {
    try {
        ...
        // Get monitor users with empcloud_user_id via employees table for org filtering
